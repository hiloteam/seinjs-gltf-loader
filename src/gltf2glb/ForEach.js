 ...
 * @private
 */
ForEach.objectLegacyAsync = async function(objects, handler) {
    if (defined(objects)) {
        for (const objectId in objects) {
            if (objects.hasOwnProperty(objectId)) {
                const object = objects[objectId];
                const value = await handler(object, objectId);
 ...
 * @private
 */
ForEach.objectAsync = async function(arrayOfObjects, handler) {
    if (defined(arrayOfObjects)) {
        const length = arrayOfObjects.length;
        for (let i = 0; i < length; i++) {
            const object = arrayOfObjects[i];
            const value = await handler(object, i);
...
ForEach.compressedImageAsync = async function(image, handler) {
    if (defined(image.extras)) {
        const compressedImages = image.extras.compressedImage3DTiles;
        for (const type in compressedImages) {
            if (compressedImages.hasOwnProperty(type)) {
                const compressedImage = compressedImages[type];
                const value = await handler(compressedImage, type);
...
ForEach.audioClip = function(gltf, handler) {
    const audios = ((gltf.extensions || {}).Sein_audioClips || {}).clips || [];
