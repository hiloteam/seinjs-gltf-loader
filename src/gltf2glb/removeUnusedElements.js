...
Remove.bufferView = function(gltf, bufferViewId) {
    ...
    if (hasExtension(gltf, 'ALI_amc_mesh_compression')) {
        ForEach.mesh(gltf, function (mesh) {
            ForEach.meshPrimitive(mesh, function (primitive) {
                if (defined(primitive.extensions) &&
                    defined(primitive.extensions.ALI_amc_mesh_compression)) {
                    if (primitive.extensions.ALI_amc_mesh_compression.bufferView > bufferViewId) {
                        primitive.extensions.ALI_amc_mesh_compression.bufferView--;
                    }
                }
            });
        });
...
getListOfElementsIdsInUse.bufferView = function(gltf) {
    ...
    ForEach.audioClip(gltf, function(audioClip) {
        if (defined(audioClip.bufferView)) {
            usedBufferViewIds[audioClip.bufferView] = true;
        }
    });
    ...
    if (hasExtension(gltf, 'ALI_amc_mesh_compression')) {
        ForEach.mesh(gltf, function(mesh) {
            ForEach.meshPrimitive(mesh, function(primitive) {
                if (defined(primitive.extensions) && defined(primitive.extensions.ALI_amc_mesh_compression)) {
                    usedBufferViewIds[primitive.extensions.ALI_amc_mesh_compression.bufferView] = true;
                }
            });
        });
    }
